...
pub fn register_quantized_types(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        pub mod quantized_dispatch {
            ...
            // ==================== Generic Dispatch (Backwards Compat) ====================
            
            /// Generic dequantize - routes to CPU by default
            #[inline]
            pub fn dequantize(
                id: QuantizedDType,
                data: &[u8],
                ...
                dequantize_cpu(id, data, output)
            }
            
            /// Generic quantize - routes to CPU by default
            #[inline]
            pub fn quantize(
                id: QuantizedDType,
                input: &[f32]
            ) -> Result<Vec<u8>> {
                quantize_cpu(id, input)
            }
