use candle_core::{DType, Device, QuantizedDType, Result, Tensor, Storage};

fn main() -> Result<()> {
    // Create an f32 tensor with 256 elements (matches Q8_0 block size)
    let data: Vec<f32> = (0..256).map(|i| (i as f32) * 0.1).collect();
    let tensor = Tensor::<Storage>::from_slice(&data, 256, &Device::Cpu)?;
    ...
    let qtypes = vec![
        ("Q4_0", QuantizedDType::Q4_0),
        ("Q4_1", QuantizedDType::Q4_1),
        ("Q5_0", QuantizedDType::Q5_0),
        ("Q5_1", QuantizedDType::Q5_1),
        ("Q8_0", QuantizedDType::Q8_0),
        // ("Q8_1", QuantizedDType::Q8_1), // Skipped - not fully implemented
        ("Q2K", QuantizedDType::Q2K),
        ("Q3K", QuantizedDType::Q3K),
        ("Q4K", QuantizedDType::Q4K),
        ("Q5K", QuantizedDType::Q5K),
        ("Q6K", QuantizedDType::Q6K),
        ("Q8K", QuantizedDType::Q8K),
    ];
    ...
    let q8_quantized = tensor.to_dtype(DType::Quantized(QuantizedDType::Q8_0))?;
    ...
    let q4k_quantized = tensor.to_dtype(DType::Quantized(QuantizedDType::Q4K))?;
    ...
    let activations = Tensor::<Storage>::from_slice(&input_data, (1, 256), &Device::Cpu)?;
    ...
    let weights_f32 = Tensor::<Storage>::from_slice(&weight_data, (256, 128), &Device::Cpu)?;
    ...
    let weights_q4k = weights_f32.to_dtype(DType::Quantized(QuantizedDType::Q4K))?;
    let weights_q8_0 = weights_f32.to_dtype(DType::Quantized(QuantizedDType::Q8_0))?;
    println!("Weight matrix (Q4K): Quantized, shape {:?}", weights_q4k.shape());
    println!("Weight matrix (Q8_0): Quantized, shape {:?}\n", weights_q8_0.shape());
    
    // Test 1: Matmul with f32 × quantized Q4K (specialized fast path)
    println!("Test 1: F32 × Quantized(Q4K) Matmul (specialized implementation)");
    ...
    // Test 2: Matmul with f32 × quantized Q8_0
    println!("\nTest 2: F32 × Quantized(Q8_0) Matmul");
    ...
    let q1 = tensor.to_dtype(DType::Quantized(QuantizedDType::Q8_0))?;
    let q2 = tensor.to_dtype(DType::Quantized(QuantizedDType::Q8_0))?;
    ...
    let q4k = tensor.to_dtype(DType::Quantized(QuantizedDType::Q4K))?;
    let q8k = q4k.to_dtype(DType::Quantized(QuantizedDType::Q8K))?;
